...
export function loadRebarScenario(): RebarNetworkInput {
  try {
    ...
    return validScenario(parsed) ? parsed : structuredClone(defaultRebarNetwork);
  } catch {
...
export function saveRebarScenario(network: RebarNetworkInput) {
  try { window.localStorage.setItem(STORAGE_KEY, JSON.stringify(network)); } catch { /* storage unavailable */ }
}
