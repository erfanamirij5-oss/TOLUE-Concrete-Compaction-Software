...
export function CombinedGradationMini({ mix, analysis }: Props) {
  ...
  const graph=(useLab:boolean)=>{
    const points=useLab?lab.points:result.points;
    ...
    return {points,width,height,px,py,xOf,yOf,line,upper,lower,polygon,outside};
  };
  ...
  const closeMain=()=>{setExpanded(false);setOpen(false)};
  const renderChart=(useLab:boolean)=>{const g=graph(useLab);return <><div className="combined-chart-shell"><svg viewBox={`0 0 ${g.width} ${g.height}`} role="img" aria-label="منحنی دانه‌بندی کل و محدوده راهنما">
    {[0,50,100].map(v=><g key={v}><line x1={g.px} x2={g.width-g.px} y1={g.yOf(v)} y2={g.yOf(v)} className="combined-mini-grid"/><text x="3" y={g.yOf(v)+3}>{v}</text></g>)}
    <polygon points={g.polygon} className="combined-envelope-fill"/><polyline points={g.upper} className="combined-envelope-limit"/><polyline points={g.lower} className="combined-envelope-limit"/>
    <polyline points={g.line} className="combined-mini-line"/>{g.points.map(p=><circle key={p.sieveMm} cx={g.xOf(p.sieveMm)} cy={g.yOf(p.passingPercent)} r="2.5" className="combined-mini-point"/>)}
  </svg></div><div className="combined-mini-legend"><span>● منحنی Blend</span><span>▨ محدوده راهنما</span><b>{g.outside===0?'داخل بازه راهنما':`${g.outside} نقطه خارج از بازه`}</b></div></>};
  const trigger=toolTarget?createPortal(<button className={`combined-tool ${open?'active':''}`} onClick={()=>setOpen(v=>!v)}>دانه‌بندی کل</button>,toolTarget):null;
  return <>{trigger}
    <FloatingToolWindow id="combined-gradation" title="منحنی دانه‌بندی کل" subtitle={`${envelope.labelFa} • غربالگری منحنی مرکب`} open={open} onClose={closeMain} initialWidth={520} initialHeight={410} initialX={360} initialY={150} minWidth={360} minHeight={300} className="gradation-floating-window">
      <div className="combined-mini combined-mini-panel">
        <div className="combined-mini-head"><div><b>منحنی دانه‌بندی کل</b><span>{envelope.labelFa} • حد بالا/پایین</span></div><div className="combined-mini-actions"><strong>{result.continuityScore}</strong><button onClick={()=>setExpanded(true)}>Design Lab</button></div></div>
        {renderChart(false)}
    ...
    <FloatingToolWindow id="gradation-design-lab" title="Combined Gradation Design Lab" subtitle="طراحی و مقایسه Blend بدون تغییر طرح اصلی" open={expanded} onClose={()=>setExpanded(false)} initialWidth={940} initialHeight={660} initialX={180} initialY={95} minWidth={620} minHeight={460} className="gradation-lab-floating-window">
      <div className="combined-mini combined-mini-expanded-content">
        <div className="combined-mini-head"><div><b>Combined Gradation Design Lab</b><span>طراحی و مقایسه Blend بدون تغییر طرح اصلی</span></div><div className="combined-mini-actions"><strong>{lab.continuityScore}</strong></div></div>
        <div className={`gradation-lab-kpis ${lab.level}`}><div><span>Continuity</span><b>{lab.continuityScore}</b></div><div><span>Packing Proxy</span><b>{lab.packingProxyPercent}٪</b></div><div><span>Void Proxy</span><b>{lab.voidProxyPercent}٪</b></div><div><span>Paste Demand</span><b>{lab.pasteDemandIndex}</b></div><div><span>Risk</span><b>{lab.riskScore}</b></div></div>
        {renderChart(true)}
        <div className="combined-analysis-panel">
          ...
          <div className={`combined-envelope-status ${graph(true).outside?'warn':'ok'}`}>{graph(true).outside?`${graph(true).outside} نقطه منحنی Blend خارج از محدوده انتخابی است.`:'کل منحنی Blend در محدوده انتخابی قرار دارد.'}</div>
